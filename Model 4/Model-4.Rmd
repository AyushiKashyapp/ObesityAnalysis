...
da20541.0001 <- da20541.0001 %>%
  select (ID, HEARN_RECODE, GENDER, AGE, RACE_RECODE, ETHGRP, COMBUILD, CONDITNS_6, CONDITNS_7,CONDITNS_4,CONDITNS_1,CONDITNS_5,EXERCISE,NOTEAT, FLTDEP, DEGREE_RECODE, HISPANIC, MARITLST,JOBSTAT_1, PHYSHLTH, MNTLHLTH, ATNDSERV )
```

```{r}
names(da20541.0001)[names(da20541.0001) == "CONDITNS_6"] <- "HYPERTENSION"
names(da20541.0001)[names(da20541.0001) == "CONDITNS_7"] <- "DIABETES"
names(da20541.0001)[names(da20541.0001) == "CONDITNS_4"] <- "ASTHMA" 
names(da20541.0001)[names(da20541.0001) == "CONDITNS_1"] <- "ARTHRITIS"
names(da20541.0001)[names(da20541.0001) == "CONDITNS_5"] <- "STROKE"
...
```{r}
odds_ratio <- exp(coef(model.lr)["LIVEALONE1"])
print(odds_ratio)
conf_int <- exp(confint(model.lr, "LIVEALONE1"))
print(conf_int)
```
...
```{r}
rpart.tree <- rpart(OBESITY ~ ., data = modeldata.train, parms  = list(split = "information"))
rpart.tree
```
